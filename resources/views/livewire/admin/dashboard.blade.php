<div class="min-h-screen w-full bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 px-0 py-8 overflow-x-hidden" style="margin: 0; max-width: 100vw;">
  <!-- Header -->
  <div class="mb-12 max-w-full px-8">
    <h1 class="text-4xl font-bold text-white mb-2">Dashboard Administratif</h1>
    <p class="text-slate-400 text-lg">Vue d'ensemble de votre établissement</p>
  </div>

  <!-- KPI Cards -->
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8 px-8">
    <!-- Total Étudiants -->
    <div class="group relative bg-gradient-to-br from-blue-600 to-blue-700 rounded-2xl p-6 overflow-hidden cursor-pointer hover:shadow-2xl transition-all duration-300">
      <div class="absolute inset-0 bg-white opacity-0 group-hover:opacity-5 transition-opacity"></div>
      <div class="relative z-10">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-blue-100 text-sm font-semibold">Total Étudiants</h3>
          <svg class="w-8 h-8 text-blue-300" fill="currentColor" viewBox="0 0 20 20">
            <path d="M13 6a3 3 0 11-6 0 3 3 0 016 0zM18 8a2 2 0 11-4 0 2 2 0 014 0zM14 15a4 4 0 00-8 0v3h8v-3zM6 8a2 2 0 11-4 0 2 2 0 014 0zM16 18v-3a5.972 5.972 0 00-.75-2.906A3.005 3.005 0 0119 15v3h-3zM4.75 12.094A5.973 5.973 0 004 15v3H1v-3a3 3 0 013.75-2.906z"></path>
          </svg>
        </div>
        <div class="text-3xl font-bold text-white mb-1" wire:key="total-students">{{ $totalStudents }}</div>
        <p class="text-blue-100 text-xs">{{ $studentsThisYear }} cette année</p>
      </div>
    </div>

    <!-- Inscriptions Validées -->
    <div class="group relative bg-gradient-to-br from-green-600 to-green-700 rounded-2xl p-6 overflow-hidden cursor-pointer hover:shadow-2xl transition-all duration-300">
      <div class="absolute inset-0 bg-white opacity-0 group-hover:opacity-5 transition-opacity"></div>
      <div class="relative z-10">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-green-100 text-sm font-semibold">Inscriptions Validées</h3>
          <svg class="w-8 h-8 text-green-300" fill="currentColor" viewBox="0 0 20 20">
            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path>
          </svg>
        </div>
        <div class="text-3xl font-bold text-white mb-1" wire:key="enrollments-validated">{{ $enrollmentsValidated }}</div>
        <p class="text-green-100 text-xs">✓ Confirmées</p>
      </div>
    </div>

    <!-- Collecte Financière -->
    <div class="group relative bg-gradient-to-br from-purple-600 to-purple-700 rounded-2xl p-6 overflow-hidden cursor-pointer hover:shadow-2xl transition-all duration-300">
      <div class="absolute inset-0 bg-white opacity-0 group-hover:opacity-5 transition-opacity"></div>
      <div class="relative z-10">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-purple-100 text-sm font-semibold">Collecté</h3>
        ...
        <div class="text-3xl font-bold text-white mb-1" wire:key="total-collected">{{ number_format($totalCollected, 0, ',', ' ') }} FCFA</div>
        <div class="flex justify-between items-center">
          <p class="text-purple-100 text-xs">{{ $collectionRate }}% collecté</p>
          <div class="w-16 h-2 bg-purple-400 rounded-full overflow-hidden">
    ...
    <!-- Montant Restant -->
    <div class="group relative bg-gradient-to-br from-orange-600 to-orange-700 rounded-2xl p-6 overflow-hidden cursor-pointer hover:shadow-2xl transition-all duration-300">
      <div class="absolute inset-0 bg-white opacity-0 group-hover:opacity-5 transition-opacity"></div>
      <div class="relative z-10">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-orange-100 text-sm font-semibold">À Collecter</h3>
        ...
        <div class="text-3xl font-bold text-white mb-1" wire:key="total-remaining">{{ number_format($totalRemaining, 0, ',', ' ') }} FCFA</div>
        <p class="text-orange-100 text-xs">À recouvrer</p>
      </div>
    </div>
  </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8 px-8">
    ...
    <div class="bg-slate-800/50 backdrop-blur border border-slate-700/50 rounded-2xl p-6 hover:border-slate-600 transition-colors">
      <div class="flex items-center justify-between mb-6">
        <div>
          <h2 class="text-xl font-bold text-white">Inscriptions par Mois</h2>
          <p class="text-slate-400 text-sm mt-1">Tendance annuelle</p>
        </div>
    ...
    <div class="bg-slate-800/50 backdrop-blur border border-slate-700/50 rounded-2xl p-6 hover:border-slate-600 transition-colors">
      <div class="flex items-center justify-between mb-6">
        <div>
          ...
          <p class="text-slate-400 text-sm mt-1">Montants collectés</p>
        </div>
    ...
    <div class="bg-slate-800/50 backdrop-blur border border-slate-700/50 rounded-2xl p-6 hover:border-slate-600 transition-colors">
      <div class="flex items-center justify-between mb-6">
        <div>
          ...
          <p class="text-slate-400 text-sm mt-1">Répartition des étudiants</p>
        </div>
    ...
    <div class="bg-slate-800/50 backdrop-blur border border-slate-700/50 rounded-2xl p-6 hover:border-slate-600 transition-colors">
      <div class="flex items-center justify-between mb-6">
        <div>
          ...
          <p class="text-slate-400 text-sm mt-1">Vue résumée</p>
        </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 px-8">
    <!-- Paiements en Retard -->
    <div class="bg-slate-800/50 backdrop-blur border border-slate-700/50 rounded-2xl p-6 hover:border-red-600/30 transition-colors">
      <div class="flex items-center justify-between mb-6">
        ...
        <span class="bg-red-600/20 text-red-300 px-3 py-1 rounded-full text-sm font-semibold" wire:key="late-count">{{ count($studentsLatePayments) }}</span>
      </div>
      <div class="space-y-3 max-h-64 overflow-y-auto">
        @forelse($studentsLatePayments as $student)
          <div class="bg-slate-700/50 rounded-lg p-3 border border-red-600/20 hover:border-red-600/50 transition-colors">
            <p class="text-white font-medium text-sm">{{ $student->nom ?? 'Nom indisponible' }}</p>
            <p class="text-red-300 text-xs mt-1">⚠️ Paiement en retard</p>
          </div>
        @empty
          <div class="text-slate-400 text-sm text-center py-8">✓ Aucun paiement en retard</div>
        @endforelse
      </div>
    </div>

    <!-- Inscriptions Attente de Paiement -->
    <div class="bg-slate-800/50 backdrop-blur border border-slate-700/50 rounded-2xl p-6 hover:border-yellow-600/30 transition-colors">
      <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
        ...
        <span class="bg-yellow-600/20 text-yellow-300 px-3 py-1 rounded-full text-sm font-semibold" wire:key="pending-count">{{ count($enrollmentsPendingPayment) }}</span>
      </div>
      <div class="space-y-3 max-h-64 overflow-y-auto">
        @forelse($enrollmentsPendingPayment as $enrollment)
          <div class="bg-slate-700/50 rounded-lg p-3 border border-yellow-600/20 hover:border-yellow-600/50 transition-colors">
            <p class="text-white font-medium text-sm">{{ $enrollment->student->nom ?? 'Étudiant' }}</p>
            <p class="text-yellow-300 text-xs mt-1">⏳ En cours de paiement</p>
          </div>
        @empty
          <div class="text-slate-400 text-sm text-center py-8">✓ Aucune inscription en attente</div>
        @endforelse
      </div>
    </div>
  </div>
